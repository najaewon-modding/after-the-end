...
public final class PlayerPositionTracker {
    ...
    public static long getOrCreateReturnDeadline(UUID playerId, long delayNanos) { return RETURN_DEADLINES.computeIfAbsent(playerId, ignored -> System.nanoTime() + delayNanos); }
    public static boolean hasReturnDeadline(UUID playerId) { return RETURN_DEADLINES.containsKey(playerId); }
    public static Long getReturnDeadline(UUID playerId) { return RETURN_DEADLINES.get(playerId); }
    public static void resetReturnDeadline(UUID playerId) { RETURN_DEADLINES.remove(playerId); }
